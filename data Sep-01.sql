...
-- Dumping structure for table bookstore.chitiethd
...
CREATE TABLE IF NOT EXISTS `chitiethd` (
  ...
  CONSTRAINT `chitiethd_ibfk_2` FOREIGN KEY (`masach`) REFERENCES `sach` (`masach`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

-- Dumping data for table bookstore.chitiethd: ~2 rows (approximately)
...
-- Dumping data for table bookstore.khachhang: ~2 rows (approximately)
...
-- Dumping structure for table bookstore.loai
...
-- Dumping data for table bookstore.loai: ~12 rows (approximately)
DELETE FROM `loai`;
...
-- Dumping structure for table bookstore.nhaxb
...
-- Dumping data for table bookstore.nhaxb: ~7 rows (approximately)
DELETE FROM `nhaxb`;
...
-- Dumping structure for table bookstore.quantri
...
-- Dumping data for table bookstore.quantri: 3 rows
DELETE FROM `quantri`;
...
-- Dumping structure for table bookstore.sach
...
CREATE TABLE IF NOT EXISTS `sach` (
  ...
  KEY `maloai` (`maloai`),
  CONSTRAINT `sach_ibfk_1` FOREIGN KEY (`manxb`) REFERENCES `nhaxb` (`manxb`) ON UPDATE CASCADE,
  CONSTRAINT `sach_ibfk_2` FOREIGN KEY (`maloai`) REFERENCES `loai` (`maloai`) ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

-- Dumping data for table bookstore.sach: ~25 rows (approximately)
DELETE FROM `sach`;
